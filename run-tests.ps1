param(
    [switch]$Headless  # if passed, emulator runs without UI
)
...
# =========================
# WAIT FOR APPIUM TO BE READY (Appium 2/3)
# =========================
...
$serial = & adb devices | Select-String "emulator" | ForEach-Object {
    ($_ -split "\s+")[0]
}
    if ($serial) { 
...
$emulators = Get-Process | Where-Object { 
